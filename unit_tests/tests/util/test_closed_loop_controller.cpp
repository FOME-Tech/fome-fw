...
TEST(ClosedLoopController, TestSetpoint) {
	...
	EXPECT_CALL(controller, getSetpoint()).WillOnce(Return(unexpected));
...
TEST(ClosedLoopController, TestSetpointSuccessPlantFail) {
	...
	// If getSetpoint returns unexpected, no other functions should be called
	EXPECT_CALL(controller, getSetpoint()).WillOnce(Return(25.0f));

	// Fail to observe plant - exec should stop there
	EXPECT_CALL(controller, observePlant()).WillOnce(Return(unexpected));
...
TEST(ClosedLoopController, TestPlantSuccessOpenLoopFail) {
	...
	EXPECT_CALL(controller, observePlant()).WillOnce(Return(75.0f));

	// Setpoint should get passed to open loop
	// It will fail, nothing else should be called
	EXPECT_CALL(controller, getOpenLoop(25.0f)).WillOnce(Return(unexpected));
...
TEST(ClosedLoopController, TestPlantOpenLoopSuccessClosedLoopFail) {
	...
	EXPECT_CALL(controller, getOpenLoop(25.0f)).WillOnce(Return(37.0f));

	// Setpoint & observation should get passed
	// But return invalid
	EXPECT_CALL(controller, getClosedLoop(25.0f, 75.0f)).WillOnce(Return(unexpected));
...
TEST(ClosedLoopController, TestAllSuccess) {
	...
	// Setpoint & observation should get passed
	// Return something valid
	EXPECT_CALL(controller, getClosedLoop(25.0f, 75.0f)).WillOnce(Return(22.0f));
