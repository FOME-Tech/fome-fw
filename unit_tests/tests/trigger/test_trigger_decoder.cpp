...
TEST(misc, testFordAspire) {
	...
	ASSERT_EQ(4, getTriggerCentral()->triggerShape.getTriggerWaveformSynchPointIndex())
...
static void testTriggerDecoder2(
		const char* msg,
		engine_type_e type,
		int synchPointIndex,
		float channel1duty,
		float channel2duty,
		float expectedGapRatio = NAN) {
...
static void
assertInjectionEventBase(const char* msg, InjectionEvent* ev, uint16_t injectorOutputMask, angle_t angleOffset) {
...
static void
assertInjectionEvent(const char* msg, InjectionEvent* ev, int injectorIndex, int eventIndex, angle_t angleOffset) {
	assertInjectionEventBase(msg, ev, (1 << injectorIndex), angleOffset);
}

static void assertInjectionEventBatch(
		const char* msg,
		InjectionEvent* ev,
		int injectorIndex,
		int secondInjectorIndex,
		int eventIndex,
		angle_t angleOffset) {
...
static void setTestBug299(EngineTestHelper* eth) {
	...
	EXPECT_CALL(*eth->mockAirmass, getAirmass(_, _)).WillRepeatedly(Return(AirmassResult{0.1008001f, 50.0f}));
...
void doTestFuelSchedulerBug299smallAndMedium(int startUpDelayMs) {
	...
	engineConfiguration->isFasterEngineSpinUpEnabled = false;
	engine->tdcMarkEnabled = false;
	eth.moveTimeForwardMs(
			startUpDelayMs); // nice to know that same test works the same with different anount of idle time on start
	setTestBug299(&eth);
	...
	// duty cycle above 75% is a special use-case because 'special' fuel event overlappes the next normal event in batch
	// mode
	...
	ASSERT_EQ(0, unitTestWarningCodeState.recentWarnings.getCount())
			<< "warningCounter#testFuelSchedulerBug299smallAndMedium";
}
...
TEST(big, testFuelSchedulerBug299smallAndLarge) {
	...
	eth.executeActions();
	ASSERT_EQ(0, unitTestWarningCodeState.recentWarnings.getCount())
			<< "warningCounter#testFuelSchedulerBug299smallAndLarge";
}
