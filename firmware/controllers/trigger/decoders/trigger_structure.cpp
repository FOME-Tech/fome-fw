...
void TriggerWaveform::addEventClamped(
		angle_t angle, bool stateParam, TriggerWheel const channelIndex, float filterLeft, float filterRight) {
	...
	if (!useOnlyRisingEdges) {
		for (size_t i = 0; i < efi::size(expectedEventCount); i++) {
			if (getExpectedEventCount((TriggerWheel)i) % 2 != 0) {
				firmwareError(
						"Trigger: should be even number of events index=%d count=%d",
						i,
						getExpectedEventCount((TriggerWheel)i));
			}
		}
	}

	bool isSingleToothOnPrimaryChannel = useOnlyRisingEdges ? getExpectedEventCount(TriggerWheel::T_PRIMARY) == 1
															: getExpectedEventCount(TriggerWheel::T_PRIMARY) == 2;
...
void TriggerWaveform::addEvent360(angle_t angle, bool state, TriggerWheel const channelIndex) {
	efiAssertVoid(
			...
			m_operationMode == FOUR_STROKE_CAM_SENSOR || m_operationMode == FOUR_STROKE_CRANK_SENSOR,
			"Not a mode for 360");
...
void TriggerWaveform::addEvent(angle_t angle, bool state, TriggerWheel const channelIndex) {
	...
	if (angle <= 0 || angle > 1) {
		firmwareError(
				...
				"angle should be positive not above 1: index=%d angle %f",
				(int)channelIndex,
				angle);
		return;
	}
	if (wave.phaseCount > 0) {
		if (angle <= previousAngle) {
			warning(ObdCode::CUSTOM_ERR_TRG_ANGLE_ORDER,
					"invalid angle order: new=%.2f/%f and prev=%.2f/%f, size=%d",
					angle,
					angle * getCycleDuration(),
					previousAngle,
					previousAngle * getCycleDuration(),
					wave.phaseCount);
...
void TriggerWaveform::setTriggerSynchronizationGap3(int gapIndex, float syncRatioFrom, float syncRatioTo) {
	...
	efiAssertVoid(
			ObdCode::OBD_PCM_Processor_Fault, gapIndex >= 0 && gapIndex < GAP_TRACKING_LENGTH, "gapIndex out of range");
...
uint16_t TriggerWaveform::findAngleIndex(TriggerFormDetails* details, angle_t targetAngle) const {
	...
	efiAssert(
			...
			engineCycleEventCount != 0 && engineCycleEventCount <= 0xFFFF,
...
void TriggerWaveform::setTriggerSynchronizationGap(float syncRatio) {
	setTriggerSynchronizationGap3(
			/*gapIndex*/ 0, syncRatio * TRIGGER_GAP_DEVIATION_LOW, syncRatio * TRIGGER_GAP_DEVIATION_HIGH);
}

void TriggerWaveform::setSecondTriggerSynchronizationGap(float syncRatio) {
	setTriggerSynchronizationGap3(
			/*gapIndex*/ 1, syncRatio * TRIGGER_GAP_DEVIATION_LOW, syncRatio * TRIGGER_GAP_DEVIATION_HIGH);
}
...
void TriggerWaveform::setThirdTriggerSynchronizationGap(float syncRatio) {
	setTriggerSynchronizationGap3(
			/*gapIndex*/ 2, syncRatio * TRIGGER_GAP_DEVIATION_LOW, syncRatio * TRIGGER_GAP_DEVIATION_HIGH);
}

void TriggerWaveform::initializeTriggerWaveform(
		operation_mode_e triggerOperationMode, const TriggerConfiguration& triggerConfig) {
	efiPrintf(
			"initializeTriggerWaveform #%d: %s)",
			(int)triggerConfig.TriggerType.type,
			getTrigger_type_e(triggerConfig.TriggerType.type));
	...
	switch (triggerConfig.TriggerType.type) {
		case trigger_type_e::TT_TOOTHED_WHEEL:
			initializeSkippedToothTrigger(
					this,
					triggerConfig.TriggerType.customTotalToothCount,
					triggerConfig.TriggerType.customSkippedToothCount,
					triggerOperationMode,
					SyncEdge::RiseOnly);
			break;
		...
		case trigger_type_e::TT_TOOTHED_WHEEL_32_2:
			...
			// this third gap is not required to sync on perfect signal but is needed to handle to reject cranking
			// transition noise
		...
		case trigger_type_e::TT_TOOTHED_WHEEL_36_2:
			...
			// second gap is not required to synch on perfect signal but is needed to handle to reject cranking
			// transition noise
		...
		default:
			setShapeDefinitionError(true);
			warning(ObdCode::CUSTOM_ERR_NO_SHAPE,
					"initializeTriggerWaveform() not implemented: %d",
					(int)triggerConfig.TriggerType.type);
	}
