...
void MultiChannelStateSequence::checkSwitchTimes(const float scale) const {
	...
	if (getSwitchTime(phaseCount - 1) != 1) {
		...
		firmwareError(
				ObdCode::CUSTOM_ERR_WAVE_1,
				"[count=%d] last switch time has to be 1/%f not %.2f/%f",
				phaseCount,
				scale,
				getSwitchTime(phaseCount - 1),
	...
	for (int i = 0; i < phaseCount - 1; i++) {
		if (getSwitchTime(i) >= getSwitchTime(i + 1)) {
			firmwareError(
					ObdCode::CUSTOM_ERR_WAVE_2,
					"invalid switchTimes @%d: %.2f/%.2f",
					i,
					getSwitchTime(i),
					getSwitchTime(i + 1));
		}
