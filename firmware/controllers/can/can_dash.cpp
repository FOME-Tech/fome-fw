...
void canMazdaRX8(CanCycle cycle) {
	if (cycle.isInterval(CI::_50ms)) {
...

		{
			...
			msg[0] = (uint8_t)(clt.value_or(0) +
							   69); // temp gauge //~170 is red, ~165 last bar, 152 centre, 90 first bar, 92 second bar
			// TODO: fixme!
			// msg[1] =
			// ((int16_t)(engine->engineState.vssEventCounter*(engineConfiguration->vehicleSpeedCoef*0.277*2.58))) &
			// 0xff;
			msg[2] = 0x00; // unknown
			msg[3] = 0x00; // unknown
			...
			if ((Sensor::getOrZero(SensorType::Rpm) > 0) &&
				(Sensor::get(SensorType::BatteryVoltage).value_or(VBAT_FALLBACK_VALUE) < 13)) {
...
static void populateFrame(Aim5f4& msg) {
	float deltaKpa = Sensor::getOrZero(SensorType::Map) - Sensor::get(SensorType::BarometricPressure).value_or(101.325);
