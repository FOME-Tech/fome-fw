...
const char* vvtNames[] = {PROTOCOL_VVT1_NAME, PROTOCOL_VVT2_NAME, PROTOCOL_VVT3_NAME, PROTOCOL_VVT4_NAME};

// these short names are part of engine sniffer protocol
static const char* const sparkShortNames[] = {
		PROTOCOL_COIL1_SHORT_NAME, "c2", "c3", "c4", "c5", "c6", "c7", "c8", "c9", "cA", "cB", "cD"};
...
static const char* const injectorShortNames[] = {
		PROTOCOL_INJ1_SHORT_NAME, "i2", "i3", "i4", "i5", "i6", "i7", "i8", "i9", "iA", "iB", "iC"};
...
static const char* const injectorStage2ShortNames[] = {
		PROTOCOL_INJ1_STAGE2_SHORT_NAME, "j2", "j3", "j4", "j5", "j6", "j7", "j8", "j9", "jA", "jB", "jC"};

static RegisteredOutputPin* registeredOutputHead = nullptr;

RegisteredOutputPin::RegisteredOutputPin(const char* registrationName, size_t pinOffset, size_t pinModeOffset)
...
// todo: one of the impediments is code generator hints handling (we need custom hints and those are not handled nice
	...
	for (size_t i = 0; i < engineConfiguration->cylindersCount; i++) {
		...
		if (isPinOrModeChanged(trailingCoilPins[i], ignitionPinMode)) {
			trailingOutput->initPin(
					trailingOutput->getName(),
					engineConfiguration->trailingCoilPins[i],
					engineConfiguration->ignitionPinMode);
		}

		NamedOutputPin* output = &enginePins.coils[i];
		if (isPinOrModeChanged(ignitionPins[i], ignitionPinMode)) {
			output->initPin(
					output->getName(), engineConfiguration->ignitionPins[i], engineConfiguration->ignitionPinMode);
		}
	}
#endif /* EFI_PROD_CODE */
}
...
void EnginePins::startInjectionPins() {
	...
	for (size_t i = 0; i < engineConfiguration->cylindersCount; i++) {
		...
		if (isPinOrModeChanged(injectionPins[i], injectionPinMode)) {
			output->initPin(
					output->getName(), engineConfiguration->injectionPins[i], engineConfiguration->injectionPinMode);
		}
		...
		if (isPinOrModeChanged(injectionPinsStage2[i], injectionPinMode)) {
			output->initPin(
					output->getName(),
					engineConfiguration->injectionPinsStage2[i],
...
bool OutputPin::getLogicValue() const {
	// Compare against 1 since it could also be INITIAL_PIN_STATE (which means logical 0, but we haven't initialized the
	// pin yet)
...
void OutputPin::initPin(
		const char* msg, brain_pin_e brainPin, pin_output_mode_e outputMode, bool forceInitWithFatalError) {
#if EFI_UNIT_TEST
	...
	if (outputMode > OM_OPENDRAIN_INVERTED) {
		firmwareError("%s invalid pin_output_mode_e %d %s", msg, outputMode, hwPortname(brainPin));
		return;
	}
	...
	if (brain_pin_is_onchip(m_brainPin)) {
		...
		if (outputMode == OM_DEFAULT || outputMode == OM_INVERTED) {
			const int logicalValue = (outputMode == OM_INVERTED) ? !actualValue : actualValue;

			// if the pin was set to logical 1, then set an error and disable the pin so that things don't catch fire
			if (logicalValue) {
				firmwareError(
						"HARDWARE VALIDATION FAILED %s: unexpected startup pin state %s actual value=%d logical "
						"value=%d mode=%s",
						msg,
						hwPortname(m_brainPin),
						actualValue,
						logicalValue,
