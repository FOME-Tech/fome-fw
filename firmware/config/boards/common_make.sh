...
make -j6 -r PROJECT_BOARD=$PROJECT_BOARD PROJECT_CPU=$PROJECT_CPU BOARD_DIR=$BOARD_DIR CRC_TYPE=HW
...
if [ "${USE_OPENBLT-no}" = "yes" ]; then
  ...
  cd bootloader; make -j6 PROJECT_BOARD=$PROJECT_BOARD PROJECT_CPU=$PROJECT_CPU BOARD_DIR=$BOARD_DIR CRC_TYPE=HW; cd ..
