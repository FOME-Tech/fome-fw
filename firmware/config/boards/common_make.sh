...
make -j$(($(nproc)*1.5)) -r PROJECT_BOARD=$PROJECT_BOARD PROJECT_CPU=$PROJECT_CPU BOARD_DIR=$BOARD_DIR
...
if [ "${USE_OPENBLT-no}" = "yes" ]; then
  ...
  cd bootloader; make -j$(($(nproc)*1.5)) PROJECT_BOARD=$PROJECT_BOARD PROJECT_CPU=$PROJECT_CPU BOARD_DIR=$BOARD_DIR; cd ..
