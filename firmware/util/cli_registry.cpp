...
static void doAddAction(const char* token, action_type_e type, Void callback, void* param) {
	...
	efiAssertVoid(
			ObdCode::CUSTOM_CONSOLE_TOO_MANY, consoleActionCount < CONSOLE_MAX_ACTIONS, "Too many console actions");
...
int handleActionWithParameter(TokenCallback* current, char* argv[], int argc) {
	...
	switch (current->parameterType) {
		...
		case STRING5_PARAMETER: {
			VoidCharPtrCharPtrCharPtrCharPtrCharPtr callbackS =
					(VoidCharPtrCharPtrCharPtrCharPtrCharPtr)current->callback;
