...
RTCDateTime convertRtcDateTimeFromEfi(efidatetime_t const* const dateTime) {
	...
	timespec.millisecond =
			(((dateTime->hour * 60) + dateTime->minute) * 60 + dateTime->second) * 1000; // ms since midnight
...
void dateToString(char* lcd_str) {
	...
	// timp.tm_mon + 1, timp.tm_mday, timp.tm_hour, timp.tm_min, timp.tm_sec); this would require a temporary mem stream
	// - see datalogging and other existing usages
