...
static DeviceType determineDevice() {
	...
	if (db) {
		...
		if (fs == 1024) {
			...
			firmwareError("1MB single bank MCU detected: please clear nDBANK option bit and reinstall FW.");
	...
	firmwareError("Unrecognized flash memory layout db=%d, size=%d", db, fs);
